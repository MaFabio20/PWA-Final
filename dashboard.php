...
// lista de tecnicos para asignar (solo tecnicos)
$tecnicos = $conn->query("SELECT id, nombre FROM usuarios WHERE rol='tecnico' ORDER BY nombre");

// obtener tickets visibles:
// - si usuario es admin: ver todos
// - si usuario es tecnico: ver tickets asignados a él y creados por él
// - si usuario normal: ver tickets creados por él
if ($role === 'admin') {
    $ticketsSQL = "SELECT t.*, u1.nombre AS creador_nombre, u2.nombre AS asignado_nombre FROM tickets t LEFT JOIN usuarios u1 ON t.creador=u1.id LEFT JOIN usuarios u2 ON t.asignado_a=u2.id ORDER BY t.fecha_creacion DESC";
} elseif ($role === 'tecnico') {
    $ticketsSQL = "SELECT t.*, u1.nombre AS creador_nombre, u2.nombre AS asignado_nombre FROM tickets t LEFT JOIN usuarios u1 ON t.creador=u1.id LEFT JOIN usuarios u2 ON t.asignado_a=u2.id WHERE t.asignado_a=$userId OR t.creador=$userId ORDER BY t.fecha_creacion DESC";
} else {
    $ticketsSQL = "SELECT t.*, u1.nombre AS creador_nombre, u2.nombre AS asignado_nombre FROM tickets t LEFT JOIN usuarios u1 ON t.creador=u1.id LEFT JOIN usuarios u2 ON t.asignado_a=u2.id WHERE t.creador=$userId ORDER BY t.fecha_creacion DESC";
}

$tickets = $conn->query($ticketsSQL);
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<main class="layout">
  <aside class="panel">
    ...
    <form action="api/create_ticket.php" method="POST" enctype="multipart/form-data">
      ...
      <!-- capture permite abrir cámara en móviles -->
  ...
  <section class="panel-list">
    ...
    <div id="ticketsContainer" class="ticketsContainer">
      <?php if ($tickets && $tickets->num_rows): ?>
        <?php while($row = $tickets->fetch_assoc()): ?>
          <article class="ticket" data-estado="<?= $row['estado'] ?>">
            <div class="ticket-row">
              ...
              <div style="text-align:right">
                ...
                <div style="margin-top:8px"><span class="status-pill <?= $row['estado']=='Abierto' ? 'status-abierto' : ($row['estado']=='En proceso' ? 'status-en_proceso' : 'status-finalizado') ?>"><?= htmlspecialchars($row['estado']) ?></span></div>
              </div>
            </div>
            ...
            <div class="ticket-info">
              ...
              <div>Creado: <?= $row['fecha_creacion'] ?></div>
            </div>
            ...
            <details class="historial">
              ...
              <ul>
                <?php
                  $hid = intval($row['id']);
                  $hres = $conn->query("SELECT h.*, u.nombre FROM historial h LEFT JOIN usuarios u ON h.usuario=u.id WHERE h.ticket_id=$hid ORDER BY h.fecha DESC");
